...
class LeanbackMedia3VideoPlayer(
    ...
    private fun prepare(uri: Uri, contentType: Int? = null) {
        ...
        val inferredContentType = contentType ?: Util.inferContentType(uri)

        val mediaSource = when (val type = inferredContentType) {
            C.CONTENT_TYPE_HLS -> {
        ...
        if (mediaSource != null) {
            contentTypeAttempts[inferredContentType] = true
