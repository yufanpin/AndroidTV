...
fun LeanbackMainContent(
    ...
    val flatIptvList = iptvGroupList.allIptvList
    ...
    val panelChannelNoSelectState = rememberLeanbackPanelChannelNoSelectState(
        onChannelNoConfirm = {
            ...
            if (channelNo in flatIptvList.indices) {
                mainContentState.changeCurrentIptv(flatIptvList[channelNo])
            }
        }
    )

    val shouldFocusVideo by remember {
        derivedStateOf {
    ...
    LaunchedEffect(shouldFocusVideo) {
        if (shouldFocusVideo) {
