...
class EpgRepository : FileCacheRepository("epg.json") {
    ...
    private suspend fun parseFromXml(
        ...
        data class EpgBuilder(
            val channel: String,
            val programmes: MutableList<EpgProgramme> = mutableListOf(),
        )

        val filteredChannelSet = filteredChannels.toHashSet()
        val epgMap = linkedMapOf<String, EpgBuilder>()
        val programmeTimeFormat = SimpleDateFormat("yyyyMMddHHmmss Z", Locale.getDefault())

        fun parseTime(time: String): Long {
            if (time.length < 14) return 0

            return programmeTimeFormat.parse(time)?.time ?: 0
        }
            ...
            when (eventType) {
                XmlPullParser.START_TAG -> {
                    if (parser.name == "channel") {
                        ...
                        if (filteredChannelSet.isEmpty() || filteredChannelSet.contains(channelName)) {
                            epgMap[channelId] = EpgBuilder(channelName)
                        }
                        ...
                        epgMap[channelId]?.programmes?.add(
                            EpgProgramme(
        ...
        return@withContext EpgList(epgMap.values.map {
            Epg(it.channel, EpgProgrammeList(it.programmes))
        })
    }
    ...
    private suspend fun fetchXml(url: String): String = withContext(Dispatchers.IO) {
        ...
        try {
            NetworkClient.http.newCall(request).execute().use { response ->
                ...
                return@use fetcher.fetch(response)
            }
        } catch (ex: Exception) {
